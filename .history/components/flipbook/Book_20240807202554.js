...
class Demobook extends React.Component {
  ...
  nextButtonClick = () => {
    if (this.flipBook && this.flipBook.pageFlip) {
      ...
      console.error('FlipBook or pageFlip is not defined');
    }
  };
  ...
  componentDidMount() {
    if (this.flipBook && this.flipBook.pageFlip) {
      try {
        const pageFlip = this.flipBook.pageFlip();
        this.setState({
          totalPage: pageFlip.getPageCount(),
        });
      } catch (error) {
        console.error('Error accessing page count:', error);
      }
    } else {
      console.error('FlipBook or pageFlip is not defined');
    }
  ...
  render() {
    return (
      <div>
        ...
        >
          <PageCover className="text-2xl">ADMISSION POLICY FOR MONTE VISTA PRIMARY SCHOOL</PageCover>
...
// Export the Demobook component as default
