...
export default function HeroSection() {
  useEffect(() => {
    const handleScroll = () => {
      ...
      const schoolBlur = Math.max(315 - scrollPosition * 0.01, 0); // Starts at 10px blur, reduces with scroll
      const schoolOpacity = Math.min(0.1 + scrollPosition * 0.001, 0.6); // Starts at 30%, goes up to 60%

      // Calculate scale for the child image
      const childScale = Math.max(1.8 - scrollPosition * 0.001, 1); // Starts at 1.5, scales down to 1
