...
export default function HeroSection() {
  useEffect(() => {
    const handleScroll = () => {
      ...
      const schoolBlur = Math.max(10 - scrollPosition * 0.02, 0); // Starts at 10px blur, reduces with scroll
      const schoolOpacity = Math.min(0.3 + scrollPosition * 0.001, 0.6); // Starts at 30%, goes up to 60%

      // Calculate scale for the child image
      const childScale = Math.max(3.5 - scrollPosition * 0.001, 1); // Starts at 1.5, scales down to 1
    ...
    <div className="relative h-screen overflow-hidden">
      ...
      <div
        ...
        style={{
          ...
          transformOrigin: 'center bottom', // Ensures the scaling happens from the bottom center
        }}
      ></div>
